...
pub const Plugin = struct {
    ...
    parsed_functions: std.json.Parsed(std.json.Value),
    args_str: []u8,
    auto_confirm: bool,
    ...
    pub fn init(
        ...
        const init_fn = library.lookup(*InitFn, "dial_plugin_init") orelse return error.SymbolNotFound; //  try loadSymbol(InitFn, library, "dial_plugin_init");
        const deinit_fn = library.lookup(*DeinitFn, "dial_plugin_deinit") orelse return error.SymbolNotFound; // try loadSymbol(DeinitFn, library, "dial_plugin_deinit");
        const schema_fn = library.lookup(*SchemaFn, "dial_plugin_schema") orelse return error.SymbolNotFound; // try loadSymbol(SchemaFn, library, "dial_plugin_schema");
        const invoke_fn = library.lookup(*InvokeFn, "dial_plugin_invoke") orelse return error.SymbolNotFound; //try loadSymbol(InvokeFn, library, "dial_plugin_invoke");
        const free_result_fn = library.lookup(*FreeResultFn, "dial_plugin_free_result") orelse return error.SymbolNotFound; //try loadSymbol(FreeResultFn, library, "dial_plugin_free_result");
        ...
        const args_str = blk: {
            ...
            break :blk try allocator.dupe(u8, root.scratch_fbs.getWritten());
        };
        ...
        var parsed = try std.json.parseFromSlice(std.json.Value, allocator, schema_str[0..schema_str_len], .{});

        const functions = try Request.parseFunctions(allocator, parsed.value);
    ...
    pub fn deinit(plugin: *Plugin) void {
        if (plugin.functions) |functions| {
            plugin.allocator.free(functions);
        }
        plugin.parsed_functions.deinit();
        plugin.deinit_fn(plugin.handle);
        plugin.allocator.free(plugin.args_str);
    ...
    fn loadSymbol(
        ...
        const symbol = std.c.dlsym(library, symbol_name) orelse {
            return error.FailedToOpenSymbol;
        };
